...
object TpchQuery {
  ...
  def outputDF(df: DataFrame, outputDir: String, className: String): Unit = {
    ...
    println("hwt save "+ outputDir + "/" + className)
  ...
  def executeQueries(sc: SparkContext, schemaProvider: TpchSchemaProvider, queryNum: Int): ListBuffer[(String, Float)] = {
    ...
    val OUTPUT_DIR: String = "hdfs://10.60.1.147:20500/tpch_10G_output/"
  ...
  def main(args: Array[String]): Unit = {
    ...
    var queryNum = 0;
	if (args.length == 0) {
		println("<Usage> args: $INPUTDIR [queryNo]")
		return
	}
    if (args.length > 1)
    ...
    val schemaProvider = new TpchSchemaProvider(INPUT_DIR)
